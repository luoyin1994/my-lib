 ...
 * 处获取接口后的数据
 *
 * 传入obj.api对象格式 {path,query,need_tk}
 * 传入obj.beforeSend没有，则往下执行；有值且为函数则执行该函数，当该函数没有返回false时，默认执行后往下执行，为false时，终止init函数
 * 传入obj.success没有，默认执行initSuccess函数
 * 传入obj.error没有，默认执行initError函数
 */
class Ajax {
  constructor(obj) {
    let apiConfig = WebApi.apiConfigs[obj.api.name];
    this.api = Object.assign(apiConfig.api, obj.api); // 合并WebApi中接口配置
  ...
  init() {
    ...
    WebApi.startProgress();
    const getApi = new GetApi(this.api); // 创建GetApi实例
    // 返回包装的promise对象
    return new Promise((resolve, reject) => {
      getApi.then(result => {
        if (result.code == WebConstant.RETURN_OK) {
          WebApi.finishProgress();
          // 成功 resolve提供对外扩展接口
          resolve(result);
        } else {
          WebApi.failProgress();
          if(this.specialError.code && this.specialError.code == result.code){
